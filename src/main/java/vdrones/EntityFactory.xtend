...
class EntityFactory {
	...
	def CfgArea newLevel(Spatial src) {
		...
		var replacer = new PlaceHolderReplacer(assetManager)
		var level = replacer.replaceTree(src.deepClone()) as Node
	...
	def private Stream<Spatial> extract(Spatial src, String groupName) {
		...
		else if (group.getQuantity() == 0)
			log.warn("empty group : {}", groupName)
...
//    }
	...
	def private void addInto(Spatial s, Collection<Location> dest) {
		...
		val v3 = s.getWorldTranslation().clone()
		// v3.y = v3.y + 2
		loc.position.set(v3)
...
//        CollisionGroups.setRecursive(b, CollisionGroups.CUBE, CollisionGroups.WALL)
		val generatingAnim = new Animation("generating", 0.5f)
		val waitingAnim = new Animation("waiting", 2.0f)
		waitingAnim.addTrack(new TrackRotateYX(2.0f))
		val exitingAnim = new Animation("exiting", 0.5f)
		val grabbedAnim = new Animation("grabbed", 0.3f)
...
//        b.attachChild(fn)
		...
		val crashingAnim = new Animation("crashing", 2.0f)
		val exitingAnim = new Animation("exiting", 0.5f)
		...
		if (spatial instanceof Geometry) {
			...
			switch (kind) {
				case "wall": {
					...
					spatial.addControl(new RigidBodyControl(new MeshCollisionShape(spatial.getMesh()), 0.0f))
					CollisionGroups.setRecursive(b, CollisionGroups.WALL, CollisionGroups.NONE)
...
class Tools {
	...
	def static void dump(Spatial s) {
		if (s instanceof Node) {
			Helper.dump(s, "")
		} else {
			System.out.print(s)
