...
class DroneInput implements ActionListener {
    ...
    override void onAction(String binding, boolean value, float tpf) {
        ...
        switch (binding) {
            case LEFT: drone.turnReq.onNext(if((value)) 1.0f else 0.0f)
            case RIGHT: drone.turnReq.onNext(if((value)) -1.0f else 0.0f)
            case FORWARD: drone.forwardReq.onNext(if((value)) 1f else 0f)
            case BACKWARD: drone.forwardReq.onNext(if((value)) -1f else 0f)
        }
    }
