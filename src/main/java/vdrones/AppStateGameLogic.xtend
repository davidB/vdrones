...
class AppStateGameLogic extends AppState0 {
    package BehaviorSubject<Float> dt = BehaviorSubject.create(0f)
    ...
    def package InfoDrone setup(Observable<Float> dt, InfoDrone drone) {
        ...
        drone.collisions.map[v| v.other.getControl(RigidBodyControl).getCollisionGroup()].subscribe(new ObserverPrint("droneCollisions"))
        drone
    }
    ...
    def package Subscription pipeAll() {
        return Subscriptions.from(Pipes.pipeA(channels.areaCfgs, geometryAndPhysic, entityFactory), // , Pipes.pipe(channels.areaCfgs, app.getStateManager().getState(AppStateLights.class))
